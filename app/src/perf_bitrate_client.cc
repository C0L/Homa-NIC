...
int main() {
    ...
    if (c2h_metadata_fd < 0 || h2c_metadata_fd < 0 || h2c_msgbuff_fd < 0) {
    ...
    if (h2c_metadata_map == NULL || c2h_metadata_map == NULL || h2c_msgbuff_map == NULL) {
    	perror("mmap failed\n");
    }
	...
	// time_trace::print_to_file("parse/perf_tp.tt");
