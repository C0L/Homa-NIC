...
if __name__ == '__main__':
    ...
    simstat_t = np.dtype([('highwater', np.uint64), ('lowwater', np.uint64), ('max', np.uint64), ('events', np.uint64), ('compcount', np.uint64), ('compsum', np.uint64), ('cycles', np.uint64), ('qo', np.uint64)])
    ...
    for trace in args.traces:
        ...
        axs[wk, 4].plot((validcycles[validcycles != 0]/(simstats['cycles'] / 10000)), label=cfg['util'])
