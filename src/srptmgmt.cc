...
void srpt_grant_pkts(hls::stream<ap_uint<58>> & header_in_i,
   ...
   static avail_pkts = OVERCOMMIT_PKTS; 
   ...
   if (!header_in_i.read_nb(header_in_raw)) {
      ...
      if (header_in.message_length > RTT_PKTS) {
         ...
         if (header_in.data_offset == 0) {
            entries[header_in.local_id] = {header_in.peer_id, header_in.rpc_id, 1, header_in.message_length - 1};
         } else {
      ...
      for (int i = 0; i < MAX_RPCS; ++i) { 
        // Is this peer already in our best list?
         ...
         if (match != -1) {
            ...
            if (best[max_entry] > entries[i]) {
               best[max_entry] = entries[i]; 
            }
         }
      }

      int min_entry = best[min_entry];
      for (int l = 0; l < 8; ++l) {
      ...
      if (best[min_entry].recv_pkts > 0
         ...
         if (best[min_entry].recv_pkts > avail_pkts) {
            ...
            entries[best[min_entry].local_id].recv_pkts -= avail_pkts;
            entries[best[min_entry].local_id].grantable_pkts -= avail_pkts;
            ...
            if ((best[min_entry].grantable_pkts - best[min_entry].recv_pkts) == 0) { 
               entries[best[min_entry].local_id] = {0xFFFFFFFF, 0xFFFFFFFF, 0xFFFFFFFF, 0xFFFFFFFF}; 
            }
            
            entries[best[min_entry]].recv_pkts = 0;
            entries[best[min_entry]].grantable_pkts -= best[min_entry].recv_pkts;
