...
#pragma HLS pipeline
    xmit_buffer_insert.write((xmit_in_t) {tmp_args.buffer.xmit_buffer[i], i, xmit_id});
  }

  homa_rpc_t homa_rpc;

  // Initial RPC state configuration depending on request vs response
  if (!tmp_args.params.id) {
    /* This is a request message */

    // Grab an unused RPC ID
    rpc_stack_next.read(homa_rpc.id);

    homa_rpc.completion_cookie = tmp_args.params.completion_cookie;
    ...
    homa_rpc.dport = tmp_args.params.dest_addr.sin6_port;
    ...
    homa_rpc.buffout = tmp_args.params.buffout;
    homa_rpc.buffin = tmp_args.params.buffin;
  } else {
    ...
    hashpack_t hashpack = {tmp_args.params.dest_addr.sin6_addr.s6_addr, tmp_args.params.id, tmp_args.params.dest_addr.sin6_port, 0};
  ...
  homa_rpc.msgout.length = tmp_args.params.length;
